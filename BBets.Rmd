...
```{r}
BrLw(numClass = 32,odds=3.0,countoDo = 637)
...
```{r}
BrLw(numClass = 38,odds=3.0,countoDo = 637)
...
betwin = unique(class)
length(betwin)
brwnlowbux <-if(length(class)-length(betwin)>0)brownlowbux=brownlowbux+1 else brownlowbux=brownlowbux-odds
}
bank[j]= brownlowbux}
```
...
### confidenceinterval `r sbank[25]` to  `r sbank[975]`
